...
// ── 탭 타입
type MainTab = 'business' | 'categories' | 'items' | 'requests' | 'suggestions' | 'community' | 'shopping' | 'bingo' | 'google' | 'deploy'
...
// ════════════════════════════════════════════
// TAB: 배포 관리 (cache_version 수동 업데이트)
// ════════════════════════════════════════════
function DeployTab() {
  ...
  const load = async () => {
    ...
    if (data) {
      ...
      data.forEach((row: any) => { v[row.key] = row.version })
      setVersions(v)
    }
    setLoading(false)
  }
  ...
  const deploy = async (key: string) => {
    ...
    const { error } = await supabase
      .from('cache_version')
      .upsert({ key, version: newVersion, updated_at: new Date().toISOString() }, { onConflict: 'key' })
    if (!error) {
      setVersions(prev => ({ ...prev, [key]: newVersion }))
      showToast(`✅ ${DEPLOY_ITEMS.find(i => i.key === key)?.label} 배포 완료 (v${newVersion})`)
    } else {
      showToast('❌ 배포 실패: ' + error.message)
    }
    setDeploying(null)
  }
  ...
  const deployAll = async () => {
    ...
    setDeploying('all')
    const updates = DEPLOY_ITEMS.map(item => ({
      key: item.key,
      version: (versions[item.key] ?? 0) + 1,
      updated_at: new Date().toISOString(),
    }))
    const { error } = await supabase.from('cache_version').upsert(updates, { onConflict: 'key' })
    if (!error) {
      const newVer: Record<string, number> = {}
      updates.forEach(u => { newVer[u.key] = u.version })
    ...
    <div>
      ...
      <button
        ...
        style={{
          ...
          background: deploying ? '#94A3B8' : '#1B6EF3',
          color: '#fff', fontSize: 15, fontWeight: 700,
          ...
          <div key={item.key} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: '#94A3B8', marginTop: 2 }}>
                {item.desc} · 현재 v{versions[item.key] ?? 1}
              </div>
            </div>
            <button
              onClick={() => deploy(item.key)}
              disabled={deploying !== null}
              style={{
                height: 36, padding: '0 16px', borderRadius: 8, border: 'none',
                background: deploying === item.key ? '#94A3B8' : '#1B6EF3',
                color: '#fff', fontSize: 13, fontWeight: 700,
                cursor: deploying !== null ? 'default' : 'pointer', flexShrink: 0,
