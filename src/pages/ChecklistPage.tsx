          ...
          {!showSearch && (
            <div style={{ position:'sticky', top:0, zIndex:30, background: '#ffffff', borderBottom:'1px solid rgba(0,131,143,0.15)' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:`${spacing[3]}px ${spacing[4]}px ${spacing[2]}px` }}>
                <span style={{ fontSize:font.size.sm, fontWeight:font.weight.bold, color: done>0 ? '#29B6D0' : '#0D3349' }}>
          ...
          <div style={{ background:'transparent', padding:`${spacing[3]}px ${spacing[3]}px` }}>
            {!showSearch && (
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:spacing[2] }}>
                <span style={{ fontSize:font.size.sm, fontWeight:font.weight.bold, color:'#29B6D0' }}>
                  {CATEGORIES.find(c=>c.id===activeCategory)?.label}
                </span>
                <span style={{ fontSize:font.size.sm, color:'#29B6D0', fontWeight:font.weight.bold }}>
                ...
                return (
                  <div key={item.id} id={`item-${item.id}`} className="list-item"
                    style={{
                      ...
                      border: checked ? `1px solid ${'#F5A623'}` : isHighlight ? '1.5px solid #29B6D0' : `1px solid ${colors.gray300}`,
                  ...
                  >
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{
                        fontSize:font.size.md, fontWeight:font.weight.medium, color:checked?'#F5A623':colors.gray800,
                      ...
                      <div style={{ display:'flex', gap:spacing[1], alignItems:'center', marginTop:3, flexWrap:'wrap' }}>
                        ...
                        {checked && dayCount>0 && (state.schedules[item.id]??[]).map(dayIdx=>(
                          <span key={dayIdx} style={{ fontSize:font.size.xs, fontWeight:font.weight.bold, background:'#F5A623', color:'#fff', borderRadius:radius.full, padding:`1px ${spacing[2]}px` }}>{dayIdx+1}일차</span>
                        ))}
                    ...
                    <button onClick={e=>{
                      ...
                      background: checked ? '#F5A623' : colors.bgCard,
