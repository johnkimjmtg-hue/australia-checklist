...
export default function ChecklistPage({ state, setState }: Props) {
  ...
  const unscheduledCount = Object.keys(state.selected).filter(
    id => !(state.schedules[id]?.length)
  ).length
  ...
  const doReset = () => {
    ...
    setModal('none')
    setScheduleSelectedItem(null)

    try {
      localStorage.removeItem('bucket-achieved')
    } catch {}
  }
      ...
      <>
        <BucketCheckView
          ...
          onEdit={() => {
            ...
            setState(next)
            try {
              localStorage.setItem('korea-receipt', JSON.stringify(next))
            } catch {}
          }}
          onDelete={doReset}
          onShare={() => {
        ...
        {showReceipt && trip && (
          <ReceiptModal
            ...
            achieved={(() => {
              try {
                return JSON.parse(localStorage.getItem('bucket-achieved') ?? '{}')
              } catch {
                return {}
              }
            })()}
    ...
    >
          ...
          {(['bucketlist', 'services'] as MainTab[]).map(tab => (
            <button
              ...
              style={{
                ...
                borderBottom:
                  mainTab === tab ? '2px solid #003594' : '2px solid transparent',
              }}
            >
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                >
                  <Icon
                    ...
                    width={15}
                    height={15}
                    color={tripLabel ? '#fff' : '#94A3B8'}
                  />
                  {tripLabel ?? '여행일정 설정'}
                </button>

                <button
                  onClick={() => setShowScheduleView(v => !v)}
                  style={{
                    height: 34,
                    padding: '0 14px',
                    borderRadius: 8,
                ...
                >
                  <Icon
                    ...
                    width={15}
                    height={15}
                    color={showScheduleView ? '#FFCD00' : '#94A3B8'}
                  />
                  일정보기
                </button>
              </div>
              ...
              return (
                ...
                >
                  {allCats.map(cat => {
                    ...
                    const catDone = allItems.filter(
                      i => i.categoryId === cat.id && state.selected[i.id]
                    ).length
                    const catUnsch = allItems.filter(
                      i =>
                        i.categoryId === cat.id &&
                        state.selected[i.id] &&
                        !(state.schedules[i.id]?.length)
                    ).length
                    ...
                    return (
                      <button
                        key={cat.id}
                        className="chip-btn"
                        onClick={() => setState(setCategory(state, cat.id))}
                        style={{
                          height: 36,
                          borderRadius: 8,
                          border: 'none',
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    gap: 16,
                    cursor: 'pointer',
                    boxShadow: '0 8px 32px rgba(0,53,148,0.30)',
                  }}
                >
                  <div
                    style={{
                ...
                return (
                  ...
                  >
                    ...
                    <Icon
                      icon={
                        ITEM_ICONS[item.id] ??
                        CAT_ICON_MAP[(item as any).categoryId] ??
                        'ph:star'
                      }
                      width={20}
                      height={20}
      ...
      {modal === 'noTrip' && (
        <AlertModal
          ...
          onConfirm={() => {
            setModal('none')
            setTimeout(handleOpenTripPicker, 100)
          }}
          onCancel={() => setModal('none')}
        />
      )}
...
function ScheduleGrid({
  ...
  useEffect(() => {
    ...
    const currentAssignedDays = state.schedules[selectedItemId] ?? []
    if (currentAssignedDays.length === 0) return

    const prevSelected = prevSelectedRef.current
    const targetDay =
      prevSelected !== selectedItemId
        ? currentAssignedDays[0]
        : currentAssignedDays[currentAssignedDays.length - 1]
  ...
  const dayCount = days.map((_, idx) =>
    allItems.filter(
      item => state.selected[item.id] && (state.schedules[item.id] ?? []).includes(idx)
    ).length
  )

  function redIntensity(count: number): string {
    if (count <= 0) return '#ffffff'
    if (count === 1) return 'rgba(220,38,38,0.25)'
    if (count === 2) return 'rgba(220,38,38,0.45)'
    if (count === 3) return 'rgba(220,38,38,0.70)'
    ...
    >
      ...
      >
        {days.map((d, idx) => {
          ...
          const bg = redIntensity(dayCount[idx])
          const strongRed = dayCount[idx] >= 3
          const textColor = strongRed ? '#fff' : '#1E293B'
          ...
          return (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                background: bg,
                color: isActive ? (isSelected ? '#1E293B' : '#003594') : textColor,
                fontSize: 11,
                fontWeight: 700,
                position: 'relative',
                textAlign: 'center',
                ...
                boxShadow: isSelected
                  ? '0 0 0 1px rgba(255,205,0,0.25), 0 2px 8px rgba(255,205,0,0.25)'
                  : isActive
                    ? '0 2px 8px rgba(0,53,148,0.20)'
                    : '0 1px 3px rgba(0,0,0,0.06)',
              }}
            >
              <div>{idx + 1}일차</div>
              <div style={{ fontSize: 10, opacity: 0.85 }}>{fmtMD(d)}</div>
          ...
          {confirmFirst && (
            <button
              ...
              style={{
                ...
                border: 'none',
                borderRadius: 6,
                background: confirmColor ?? '#003594',
                color: '#fff',
                fontWeight: 700,
                fontSize: 15,
                cursor: 'pointer',
