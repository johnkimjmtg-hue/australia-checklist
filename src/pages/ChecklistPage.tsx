  ...
  return (
    <div style={{ 
      ...
      <style>{`
        ...
        .list-item:active { background: rgba(0,131,143,0.05) !important; }
      ...
      {mainTab==='services' ? (
        ...
        <div style={{ paddingBottom: embedded ? 20 : 130, background: 'transparent', minHeight: '100dvh' }}>
          ...
          {/* ── 서브헤더 + 카테고리 (sticky 고정) ── */}
          {!showSearch && (
            <div style={{ position:'sticky', top:0, zIndex:30, background: 'linear-gradient(180deg, #E0F7FA 0%, #80DEEA 35%, #4DD0E1 65%, #26C6DA 100%)', borderBottom:'1px solid rgba(0,131,143,0.15)' }}>
              {/* 멘트 + 일정설정 + 일정보기 */}
          ...
          <div style={{ background:'transparent', padding:`${spacing[3]}px ${spacing[3]}px` }}>
            ...
            {activeCategory==='custom'&&!showSearch&&(
              <div style={{ display:'flex', gap:spacing[2], marginBottom:spacing[3], alignItems:'center' }}>
                <div style={{ flex:1, display:'flex', alignItems:'center', gap:spacing[2], background:'#EFFCFC', borderRadius:radius.sm, padding:`0 ${spacing[3]}px`, border:'1px solid rgba(0,131,143,0.2)', height:44 }}>
                  ...
                  <input ref={inputRef} value={customLabel} onChange={e=>setCustomLabel(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleAddCustom()} placeholder="직접 추가"
                    style={{ flex:1, border:'none', outline:'none', fontSize:font.size.md, color:'#0D3349', background:'transparent', fontFamily:ff }} />
                </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:spacing[5] }}>
              {(searchResults??catItems).map(item => {
                ...
                return (
                  ...
                  >
                    {/* 이미지 */}
                    <div style={{ width:56, height:56, borderRadius:radius.sm, flexShrink:0, background:'rgba(0,131,143,0.08)', border:'1px solid rgba(0,131,143,0.2)', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center' }}>
                      {db?.image_url
                        ? <img src={db.image_url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                        : <Icon icon={db?.icon ?? CAT_ICON_MAP[item.categoryId] ?? 'ph:star'} width={26} height={26} color={'#7BAAB5'} />
                      }
                    </div>
                    {/* 텍스트 */}
                      ...
                      <div style={{ display:'flex', gap:spacing[1], alignItems:'center', marginTop:3, flexWrap:'wrap' }}>
                        {checked && dayCount===0 && (
                          <span
                            ...
                            style={{ fontSize:font.size.xs, color:'#F59E0B', fontWeight:font.weight.bold, cursor:'pointer', textDecoration:'underline', textUnderlineOffset:2 }}
                          >일정 미지정</span>
                        )}
                        ...
                        {region && <span style={{ fontSize:font.size.xs, color:'#7BAAB5', fontWeight:font.weight.medium, marginLeft:'auto' }}>📍 {region.label}</span>}
                      </div>
                    </div>
                    {/* 체크박스 */}
                    <button onClick={e=>{
                      ...
                      border: checked ? 'none' : '1.5px solid rgba(0,131,143,0.15)',
                      background: checked ? colors.success : '#EFFCFC',
      ...
      {/* ── 하단 고정 영역 ── */}
      {!embedded && <div style={{ position:'fixed', bottom:0, left:'50%', transform:'translateX(-50%)', width:'100%', maxWidth:430, background:'#EFFCFC', zIndex:40 }}>


        <div style={{
          display:'flex', padding:`${spacing[1]}px ${spacing[3]}px 0`,
          borderTop:'1.5px solid rgba(0,131,143,0.15)',
          ...
          {onGoHome && (
            <button className="nav-btn" onClick={onGoHome}
              ...
              <Icon icon="ph:house" width={22} height={22} color={'#7BAAB5'} />
              <span style={{ fontSize:12, fontWeight:font.weight.medium, color:'#7BAAB5' }}>홈</span>
            </button>
          )}
          {TABS.map(tab=>{
            ...
            return (
              <button key={tab.id} className="nav-btn" onClick={()=>handleTabClick(tab.id)}
                ...
                <div style={{ position:'relative', display:'inline-flex' }}>
                  <Icon icon={tab.icon} width={22} height={22} color={isActive?'#00838F':'#7BAAB5'} />
                  {badge>0&&(
                    <div style={{
                ...
                <span style={{ fontSize:12, fontWeight:isActive?font.weight.bold:font.weight.medium, color:isActive?'#00838F':'#7BAAB5' }}>{tab.label}</span>
            ...
            <div style={{ padding:`${spacing[4]}px ${spacing[4]}px ${spacing[4]}px` }}>
              <div style={{ display:'flex', gap:spacing[1], flexWrap:'wrap', marginBottom:spacing[3], alignItems:'center' }}>
                {(selProduct.tags??[]).map((tag:string)=>(
                  <span key={tag} style={{ fontSize:font.size.xs, fontWeight:font.weight.bold, padding:`2px ${spacing[2]}px`, borderRadius:radius.sm, background:TAG_COLOR[tag]?.bg??'rgba(0,131,143,0.12)', color:TAG_COLOR[tag]?.color??'#00838F' }}>{tag}</span>
                ))}
                {selProduct.price_range&&<span style={{ fontSize:font.size.xs, fontWeight:font.weight.bold, padding:`2px ${spacing[2]}px`, borderRadius:radius.sm, background:'rgba(0,131,143,0.10)', color:PRICE_COLOR[selProduct.price_range]??'#00838F', border:'1px solid rgba(0,131,143,0.2)', marginLeft:'auto' }}>{selProduct.price_range} · {PRICE_LABEL[selProduct.price_range]}</span>}
              </div>
              ...
              <button onClick={()=>setSelProduct(null)} style={{ width:'100%', height:48, borderRadius:radius.md, border:'1px solid rgba(0,131,143,0.2)', background:'#EFFCFC', color:'#1565A0', fontSize:font.size.md, fontWeight:font.weight.bold, cursor:'pointer', fontFamily:ff }}>닫기</button>
            </div>
          </div>
        </>
      )}
      ...
      {detailItem&&(
        <div onClick={()=>setDetailItem(null)} style={{ position:'fixed', inset:0, zIndex:900, background:'rgba(0,0,0,0.45)', fontFamily:ff }}>
          <div onClick={e=>e.stopPropagation()} style={{
            position:'fixed', bottom:16, left:'50%', transform:'translateX(-50%)',
            width:'calc(100% - 32px)', maxWidth:398, background:'linear-gradient(180deg, #E0F7FA 0%, #80DEEA 35%, #4DD0E1 65%, #26C6DA 100%)',
            ...
            <div style={{ padding:`${spacing[4]}px ${spacing[4]}px ${spacing[4]}px` }}>
              ...
              {(detailItem.related_product_ids?.length??0)>0&&(()=>{
                ...
                return prods.length>0 ? (
                  <div style={{ marginBottom:spacing[4] }}>
                    <div style={{ fontSize:font.size.xs, fontWeight:font.weight.bold, color:'#1565A0', marginBottom:spacing[2] }}>🛍️ 관련 상품</div>
                    <div style={{ display:'flex', flexDirection:'column', gap:spacing[2] }}>
                      {prods.map(prod=>(
                        <button key={prod.id} onClick={()=>setSelProduct(prod)} style={{ display:'flex', alignItems:'center', gap:spacing[3], background:'rgba(0,131,143,0.08)', border:'1px solid rgba(0,131,143,0.2)', borderRadius:radius.md, padding:`${spacing[3]}px`, cursor:'pointer', textAlign:'left', width:'100%' }}>
                          {prod.image_url && <img src={prod.image_url} alt="" style={{ width:44, height:44, borderRadius:radius.sm, objectFit:'cover', flexShrink:0 }} />}
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ fontSize:font.size.sm, fontWeight:font.weight.bold, color:'#0D3349', marginBottom:2 }}>{prod.name}</div>
                            <div style={{ fontSize:font.size.xs, color:'#7BAAB5' }}>{prod.brand}</div>
                          </div>
                          <Icon icon="ph:arrow-right" width={16} height={16} color={'#7BAAB5'} />
                        </button>
                      ))}
                    </div>
                  </div>
      ...
      {showMenu && (
        <>
          ...
          <div style={{
            ...
            background:'#EFFCFC', borderRadius:radius.xl,
            ...
            <div style={{ fontSize:font.size.md, fontWeight:font.weight.bold, color:'#0D3349', marginBottom:spacing[3], paddingBottom:spacing[3], borderBottom:'1px solid rgba(0,131,143,0.2)' }}>호주가자</div>
            {[
              ...
              <button key={item.tab} onClick={()=>{ setTermsTab(item.tab); setShowMenu(false) }} style={{
                ...
                <Icon icon={item.icon} width={20} height={20} color={'#1565A0'} />
                <span style={{ fontSize:font.size.md, color:'#0D3349', fontWeight:font.weight.medium }}>{item.label}</span>
                <Icon icon="ph:caret-right" width={16} height={16} color={'#7BAAB5'} style={{ marginLeft:'auto' }} />
              </button>
            ))}
          </div>
        </>
...
function CalendarModal({ state, trip, allItems, onClose }: { state:AppState; trip:TripInfo; allItems:any[]; onClose:()=>void }) {
  ...
  function dayColor(count:number):string { if(count<=0) return '#EFFCFC'; if(count===1) return '#D1FAE5'; if(count===2) return '#FEF08A'; if(count===3) return '#FED7AA'; return '#FCA5A5' }
  ...
  return (
    <div style={{ background:'#EFFCFC', padding:`${spacing[3]}px ${spacing[3]}px ${spacing[4]}px`, fontFamily:font.family }}>
      {/* 헤더 */}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:spacing[3] }}>
        <span style={{ fontSize:font.size.sm, fontWeight:font.weight.bold, color:'#0D3349' }}>📅 일정 현황</span>
        <div style={{ display:'flex', alignItems:'center', gap:spacing[3] }}>
          {/* 범례 */}
          <div style={{ display:'flex', gap:spacing[2] }}>
            {[['없음','#fff'],['1개','#D1FAE5'],['2개','#FEF08A'],['3개','#FED7AA'],['4+','#FCA5A5']].map(([label,color])=>(
              <div key={label} style={{ display:'flex', alignItems:'center', gap:3 }}>
                <div style={{ width:8, height:8, borderRadius:2, background:color, border:'1px solid rgba(0,131,143,0.10)' }}/>
                <span style={{ fontSize:9, color:'#7BAAB5' }}>{label}</span>
              </div>
            ))}
          </div>
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:'#7BAAB5', fontSize:16, padding:0, lineHeight:1 }}>✕</button>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', marginBottom:4 }}>
        {DOW_LABELS.map((d,i)=>(
          <div key={d} style={{ textAlign:'center', fontSize:9, fontWeight:font.weight.bold, padding:'3px 0',
            color: i===0?'#E05050':i===6?'#4477CC':'#7BAAB5' }}>
            {d}
          </div>
        ))}
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:3 }}>
        {cells.map((dayIdx, ci) => {
          ...
          const bg = isActive ? '#00838F' : dayColor(dayCount[dayIdx])
          ...
          return (
            <button key={ci} onClick={()=>setActiveDayIdx(isActive?null:dayIdx)} style={{
              ...
              border: isActive ? '1.5px solid #00838F' : `1px solid ${'#B2EBF2'}`,
              background: bg,
              color: isActive ? '#fff' : '#0D3349',
              ...
              <div style={{ fontSize:font.size.xs, fontWeight:font.weight.bold,
                color: isActive?'#fff': colIdx===0?'#E05050':colIdx===6?'#4477CC':'#0D3349' }}>
                {dateNum}
              </div>
              <div style={{ fontSize:8, opacity:0.7, color:isActive?'#fff':'#7BAAB5' }}>
                {dayIdx+1}일
              </div>
              {dayCount[dayIdx]>0&&!isActive&&(
                <span style={{ position:'absolute', top:2, right:3, fontSize:8, fontWeight:font.weight.bold, color:'#1565A0' }}>{dayCount[dayIdx]}</span>
              )}
            </button>
          )
        })}
      ...
      <div style={{ marginTop:spacing[3], minHeight:40 }}>
        {activeDayIdx===null ? (
          <p style={{ ...T.sm, textAlign:'center', padding:`${spacing[2]}px 0`, color:'#7BAAB5' }}>날짜를 눌러 일정을 확인하세요</p>
        ) : dayItems.length===0 ? (
          <p style={{ ...T.sm, textAlign:'center', padding:`${spacing[2]}px 0`, color:'#7BAAB5' }}>이 날 할당된 항목이 없어요</p>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:spacing[1] }}>
            <div style={{ fontSize:font.size.xs, fontWeight:font.weight.bold, color:'#1565A0', marginBottom:4 }}>
              {activeDayIdx+1}일차 · {fmtMD(days[activeDayIdx])} ({dayItems.length}개)
            </div>
            {dayItems.map(item=>(
              <div key={item.id} style={{ display:'flex', alignItems:'center', gap:spacing[2], padding:`${spacing[2]}px ${spacing[3]}px`, background:'#EFFCFC', borderRadius:radius.sm, border:'1px solid rgba(0,131,143,0.2)' }}>
                <Icon icon="ph:check-circle-fill" width={13} height={13} color={'#00838F'} />
                <span style={{ fontSize:font.size.sm, color:'#0D3349', fontWeight:font.weight.medium }}>
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background:'#EFFCFC', borderRadius:radius.lg, padding:`${spacing[6]}px ${spacing[5]}px`,
        zIndex:601, width:'calc(100% - 48px)', maxWidth:300, textAlign:'center',
        animation:'scaleIn 0.22s ease', fontFamily:font.family,
        boxShadow:'0 8px 32px rgba(0,0,0,0.15)',
      }}>
        <p style={{ fontSize:font.size.lg, fontWeight:font.weight.bold, color:'#0D3349', marginBottom:message?spacing[2]:spacing[5], lineHeight:1.5 }}>{title}</p>
        {message&&<p style={{ fontSize:font.size.sm, color:'#1565A0', marginBottom:spacing[5], lineHeight:1.6 }}>{message}</p>}
        <div style={{ display:'flex', gap:spacing[2] }}>
          {confirmFirst&&<button onClick={onConfirm} style={{ flex:2, height:48, border:'none', borderRadius:radius.sm, background:'rgba(0,131,143,0.10)', color:confirmColor??'#00838F', fontWeight:font.weight.bold, fontSize:font.size.md, cursor:'pointer', fontFamily:font.family }}>{confirmLabel}</button>}
          {!hideCancel&&<button onClick={onCancel} style={{ flex:1, height:48, border:'1px solid rgba(0,131,143,0.2)', borderRadius:radius.sm, background:'#EFFCFC', color:'#1565A0', fontWeight:font.weight.medium, fontSize:font.size.sm, cursor:'pointer', fontFamily:font.family }}>취소</button>}
          {!confirmFirst&&<button onClick={onConfirm} style={{ flex:2, height:48, border:'none', borderRadius:radius.sm, background:confirmColor===colors.danger?colors.dangerLight:'rgba(0,131,143,0.12)', color:confirmColor??'#00838F', fontWeight:font.weight.bold, fontSize:font.size.md, cursor:'pointer', fontFamily:font.family }}>{confirmLabel}</button>}
        </div>
      </div>
    </>
  )
...
function MiniCalendar({ year, month, selected, minDate, onSelect }: { year:number; month:number; selected:string; minDate?:string; onSelect:(d:string)=>void }) {
  ...
  return (
    <div style={{ width:'100%' }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', marginBottom:4 }}>
        {DAYS.map((d,i)=><div key={d} style={{ textAlign:'center', fontSize:font.size.xs, fontWeight:font.weight.bold, padding:'4px 0', color:i===0?'#E05050':i===6?'#4477CC':'#7BAAB5' }}>{d}</div>)}
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:'3px 0' }}>
        {cells.map((day,idx)=>{
          ...
          return (
            <div key={idx} onClick={()=>!isDisabled&&onSelect(dateStr)} style={{ textAlign:'center', padding:'6px 0', borderRadius:radius.sm, cursor:isDisabled?'default':'pointer', background:isSelected?'#00838F':'transparent', color:isDisabled?'#B2EBF2':isSelected?'#fff':dayOfWeek===0?'#E05050':dayOfWeek===6?'#4477CC':'#0D3349', fontSize:font.size.sm, fontWeight:isSelected||isToday?font.weight.bold:font.weight.regular, border:isToday&&!isSelected?'1.5px solid #00838F':'1.5px solid transparent', boxSizing:'border-box' }}>{day}</div>
          )
        })}
      </div>
    </div>
...
function TripPickerModal({ step, startDate, onSelect, onReset, onClose }: { step:'start'|'end'; startDate:string; onSelect:(v:string)=>void; onReset:()=>void; onClose:()=>void }) {
  ...
  return (
    <>
      ...
      <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:'calc(100% - 32px)', maxWidth:340, background:'#EFFCFC', borderRadius:radius.lg, padding:`${spacing[5]}px`, zIndex:501, animation:'scaleIn 0.2s ease', maxHeight:'90vh', overflowY:'auto', fontFamily:font.family }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:spacing[3] }}>
          <button onClick={()=>{ if(month===1){setYear(y=>y-1);setMonth(12)} else setMonth(m=>m-1) }} style={{ width:32, height:32, borderRadius:radius.sm, border:'1px solid rgba(0,131,143,0.2)', background:'#EFFCFC', cursor:'pointer', fontSize:14, color:'#00838F', fontWeight:font.weight.bold }}>‹</button>
          <div style={{ ...T.h4 }}>{year}년 {month}월</div>
          <button onClick={()=>{ if(month===12){setYear(y=>y+1);setMonth(1)} else setMonth(m=>m+1) }} style={{ width:32, height:32, borderRadius:radius.sm, border:'1px solid rgba(0,131,143,0.2)', background:'#EFFCFC', cursor:'pointer', fontSize:14, color:'#00838F', fontWeight:font.weight.bold }}>›</button>
        </div>
        <MiniCalendar year={year} month={month} selected={selected} minDate={minDate} onSelect={d=>setSelected(d)} />
        <div style={{ marginTop:spacing[3], padding:`${spacing[3]}px`, borderRadius:radius.sm, background:selected?'rgba(0,131,143,0.12)':'rgba(0,131,143,0.08)', textAlign:'center', fontSize:font.size.sm, fontWeight:font.weight.bold, color:selected?'#00838F':'#7BAAB5' }}>
          {selected?`📅 ${selected}`:'날짜를 선택해주세요'}
        </div>
        <button onClick={()=>selected&&onSelect(selected)} style={{ width:'100%', height:48, borderRadius:radius.sm, border:'none', cursor:selected?'pointer':'default', background:selected?'#00838F':'rgba(0,131,143,0.08)', color:selected?'#fff':'#7BAAB5', fontSize:font.size.md, fontWeight:font.weight.bold, marginTop:spacing[3], fontFamily:font.family }}>
          {isStart?'다음 →':'완료'}
        </button>
