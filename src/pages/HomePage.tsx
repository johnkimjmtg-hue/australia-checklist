...
import { TripInfo, AppState, loadState, getTripDays, setSchedule, toggleItem } from '../store/state'
import { ITEMS, CATEGORIES } from '../data/checklist'
import BucketView from '../components/home/BucketView'
...
type Props = { trip: TripInfo; state: AppState; setState: (s: AppState) => void; onNavigate: (tab: Tab) => void; onChangeDates: () => void }
type CityData = {
  temp: number | null
...
export default function HomePage({ trip, state: stateProp, setState, onNavigate, onChangeDates }: Props) {
    ...
    <div style={{
      ...
      <div style={{ flex:1, padding:'0 18px 40px', overflowY:'auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          {MENUS.map((m, i) => (
            <div key={m.id} className="menu-card-hover card-anim" onClick={() => (m as any).local ? setActiveView('bucket') : onNavigate(m.id)}
