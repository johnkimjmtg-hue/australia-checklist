    ...
    <div style={{ position:'relative' }}>
        ...
        <div style={{
          ...
          {suggestions.map((s: any, i: number) => (
            <div key={i} onClick={() => handleSelect(s)} style={{
              ...
              onMouseEnter={e => (e.currentTarget.style.background='rgba(41,182,208,0.12)')}
              onMouseLeave={e => (e.currentTarget.style.background=colors.bgCard)}
            >
              <Icon icon="ph:map-pin-simple" width={14} height={14} color={'#29B6D0'} />
              {s.placePrediction?.text?.text || ''}
            </div>
          ))}
          <div onClick={() => { setSuggestions([]); setManualMode(true); onChange('') }} style={{
            padding:'10px 14px', fontSize:font.size.sm, cursor:'pointer',
            color:'#29B6D0', fontWeight:font.weight.bold,
            display:'flex', alignItems:'center', gap:8,
            borderTop:`1px solid ${colors.border}`, background:'rgba(41,182,208,0.12)',
          }}
            onMouseEnter={e => (e.currentTarget.style.opacity='0.8')}
            onMouseLeave={e => (e.currentTarget.style.opacity='1')}
          >
            <Icon icon="ph:pencil-simple" width={14} height={14} color={'#29B6D0'} />
            찾는 주소가 없어요 — 직접 입력하기
          </div>
        </div>
      )}
      {!loadingAc && !suggestions.length && value.length >= 3 && (
        <button onClick={() => setManualMode(true)} style={{
          background:'none', border:'none', fontSize:11, color:'#29B6D0',
...
function RequestForm({ onClose }: { onClose: () => void }) {
  ...
  if (done) return (
    <div style={{ textAlign:'center', padding:'32px 0' }}>
      ...
      <button onClick={onClose} style={{ width:'100%', height:48, background:'rgba(41,182,208,0.12)', color:'#29B6D0', border:`1px solid ${colors.border}`, borderRadius:radius.sm, fontSize:font.size.md, fontWeight:font.weight.bold, cursor:'pointer', fontFamily:ff }}>확인</button>
    </div>
  )
  ...
  return (
    <div>
        ...
        <div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6 }}>
            {businessCats.map(cat => (
              <button key={cat.id} onClick={() => set('category', cat.id)} style={{ height:36, borderRadius:radius.sm, border: form.category===cat.id ? `1.5px solid ${'#29B6D0'}` : `1px solid ${colors.border}`, cursor:'pointer', background: form.category===cat.id ? 'rgba(41,182,208,0.12)' : colors.bgCard, color: form.category===cat.id ? '#29B6D0' : colors.textSecondary, fontSize:font.size.sm, fontWeight: form.category===cat.id ? font.weight.bold : font.weight.regular, fontFamily:ff }}>{cat.label}</button>
            ))}
          </div>
        </div>
      ...
      <button onClick={handleSubmit} disabled={submitting} style={{ width:'100%', marginTop:16, height:50, background:'#29B6D0', color:'#fff', border:'none', borderRadius:radius.sm, fontSize:font.size.md, fontWeight:font.weight.bold, cursor: submitting?'default':'pointer', opacity: submitting?0.7:1, fontFamily:ff }}>{submitting ? '제출 중...' : '등록 신청하기'}</button>
    </div>
  )
}
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:spacing[2], marginBottom:spacing[3] }}>
          {(['all', 'korean', 'bookmarks', 'emergency'] as ServiceTab[]).filter(tab => tab !== 'korean').map(tab => {
            ...
            const activeColor = tab === 'emergency' ? colors.danger : '#29B6D0'
            const iconColor = isActive ? (tab === 'emergency' ? colors.danger : '#29B6D0') : tab === 'korean' ? '#EA580C' : tab === 'bookmarks' ? '#FFB800' : tab === 'emergency' ? colors.danger : colors.textTertiary
            return (
              <button key={tab} onClick={() => { setServiceTab(tab); setCategory(''); setSearch('') }}
                ...
                style={{
                  ...
                  background: isActive ? (tab === 'emergency' ? colors.dangerLight : tab === 'korean' ? '#FFF7ED' : 'rgba(41,182,208,0.12)') : colors.bgCard,
            ...
            return (
              <button key={cat.id} className="chip-btn svc-btn"
                ...
                style={{
                  ...
                  background: isActive ? '#29B6D0' : colors.bgCard,
                  color: isActive ? '#fff' : colors.gray600,
                  fontSize:font.size.sm, fontWeight:font.weight.bold,
                  cursor:'pointer', flexShrink:0,
                  padding:`0 ${spacing[3]}px`,
                  border: isActive ? `2px solid ${'#29B6D0'}` : `1px solid ${colors.gray300}`,
      ...
      <div style={{ padding:`${spacing[4]}px ${spacing[4]}px 0` }}>
        ...
        {serviceTab === 'bookmarks' && (
          bookmarked.length === 0 ? (
            ...
            <>
              <SectionLabel icon="ph:bookmark-simple-fill" label={`내 북마크 (${bookmarked.length})`} color={'#29B6D0'} />
