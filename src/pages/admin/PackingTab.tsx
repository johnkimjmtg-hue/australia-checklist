// ─────────────────────────────────────────────
// PackingTab.tsx
// src/pages/admin/PackingTab.tsx
// ─────────────────────────────────────────────
...
// CategoriesTab
// ════════════════════════════════════════════
export function PackingCategoriesTab({ cats, setCats }: { cats: Cat[]; setCats: (cats: Cat[]) => void }) {
  ...
  async function addCat() {
    ...
    setSaving(true)
    const { error } = await supabase.from('packing_categories').insert({ id, label: newLabel.trim(), emoji, sort_order })
  ...
  async function deleteCat(id: string) {
    ...
    await supabase.from('packing_items').delete().eq('category_id', id)
    await supabase.from('packing_categories').delete().eq('id', id)
  ...
  async function updateCat(id: string, field: 'label' | 'emoji', val: string) {
    ...
    await supabase.from('packing_categories').update({ [field]: val }).eq('id', id)
  }
  ...
  async function handleDrop(toIdx: number) {
    ...
    await Promise.all(reordered.map(c => supabase.from('packing_categories').update({ sort_order: c.sort_order }).eq('id', c.id)))
    showToast('순서 저장됨')
  }
...
// ItemsTab
// ════════════════════════════════════════════
export function PackingItemsTab({ cats, items, setItems }: { cats: Cat[]; items: Item[]; setItems: (items: Item[]) => void }) {
  ...
  async function addItem() {
    ...
    const { error } = await supabase.from('packing_items').insert(insertData)
  ...
  async function handleDrop(toIdx: number) {
    ...
    await Promise.all(reordered.map(item => supabase.from('packing_items').update({ sort_order: item.sort_order }).eq('id', item.id)))
    showToast('순서 저장됨')
  }
